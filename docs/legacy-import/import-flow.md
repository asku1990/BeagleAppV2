# Legacy Import Flow (Phases 1, 1.5, 2, 3, 5-spec)
...
- Optional full bootstrap (`auth:bootstrap-admin` -> `seed:show-result-definitions` -> `seed:show-workbook-import-schema` -> `phase1` -> `phase1.5` -> `phase2` -> `phase3` -> `phase5`): `pnpm import:bootstrap`
...
3. `phase2` imports legacy AJOK trial source rows into frozen mirror tables
...
6. `phase5-trial-runtime-projection` is a documentation-only trial projection
...
7. `phase5` rebuilds runtime trial projection tables from the phase2 mirror.
   It creates runtime `TrialEvent`, `TrialEntry`, `TrialEra`, and
   `TrialEraLisatieto` rows using the documented projection rules.

8. `seed:show-result-definitions` bootstraps canonical `ShowResultDefinition` rows used by both
...
9. `seed:show-workbook-import-schema` bootstraps `ShowWorkbookColumnRule` metadata used by the
...
- The entire legacy import flow is one-shot: `seed:show-result-definitions`,
  `seed:show-workbook-import-schema`, `phase1`, `phase1.5`, `phase2`, `phase3`,

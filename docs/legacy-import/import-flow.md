# Legacy Import Flow (Phases 1, 1.25, 1.5, 2, 3, 5-spec)
...
- `docs/legacy-import/phase1.25.md`
...
- Phase 1.25 (disease data): `pnpm import:phase1.25`
...
- Optional full bootstrap (`auth:bootstrap-admin` -> `seed:show-result-definitions` -> `seed:show-workbook-import-schema` -> `phase1` -> `phase1.25` -> `phase1.5` -> `phase2` -> `phase3` -> `phase5`): `pnpm import:bootstrap`
...
2. `phase1.25` imports disease data:
...
4. `phase2` imports legacy AJOK trial source rows into frozen mirror tables
   only (`legacy_akoeall`, `legacy_bealt`, `legacy_bealt0`, `legacy_bealt1`,
   `legacy_bealt2`, `legacy_bealt3`).

5. `import:trials:validate-mirror` validates the frozen trial mirror tables
   before runtime trial projection. It is read-only and does not create an
   `ImportRun`.

6. `phase3` imports show rows into canonical show tables (`ShowEvent`, `ShowEntry`,
...
7. `phase5-trial-runtime-projection` is a documentation-only trial projection
   specification. It freezes how mirror rows are projected into redesigned
   runtime trial tables in later implementation phases. It does not run writes
   or create an `ImportRun`.

8. `phase5` rebuilds runtime trial projection tables from the phase2 mirror.
   It creates runtime `TrialEvent`, `TrialEntry`, `TrialEra`, and
   `TrialEraLisatieto` rows using the documented projection rules.

9. `seed:show-result-definitions` bootstraps canonical `ShowResultDefinition` rows used by both
...
10. `seed:show-workbook-import-schema` bootstraps `ShowWorkbookColumnRule` metadata used by the
...
- The entire legacy import flow is one-shot: `seed:show-result-definitions`,
  `seed:show-workbook-import-schema`, `phase1`, `phase1.25`, `phase1.5`, `phase2`, `phase3`,
  `phase5`, and `import:bootstrap` all belong to the same initial canonical
  bootstrap/migration.
...
- `phase1.25` -> `kind=LEGACY_PHASE1_25`
- `phase1.5` -> `kind=LEGACY_PHASE1_5`
- `phase2` -> `kind=LEGACY_TRIAL_MIRROR`
- `phase3` -> `kind=LEGACY_PHASE3`
- `phase5` -> `kind=LEGACY_TRIAL_PROJECTION`
...
(`seed:show-result-definitions` -> `seed:show-workbook-import-schema` -> `phase1` -> `phase1.25` -> `phase1.5` -> `phase2` -> `phase3`).
Treat it as a one-time bootstrap/migration flow, not as an ongoing sync,
replay, or upgrade pipeline.

Phase 4 trial projection is a design/spec step in this same migration effort.
It has no runtime command and therefore is not part of the executable bootstrap chain.

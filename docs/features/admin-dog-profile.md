...
- `EPI-luku` and `EPITEKSTI` are calculated on demand in shared server code from imported `KoiranSairaus` disease rows (`epi`, `lepis`, `lepik`, `lepit`) plus the bounded pedigree ancestry loaded for the profile dog. Real disease rows use canonical `Dog.sireId` and `Dog.damId`; anonymous rows with no `dogId` can contribute only relationship evidence through their resolved source parents.

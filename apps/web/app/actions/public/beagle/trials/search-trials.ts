...
export type SearchBeagleTrialsActionResult = {
  data: BeagleTrialSearchResponse | null;
  hasError: boolean;
  status: number;
  error?: string;
};
...
export async function searchBeagleTrialsAction(
  input: BeagleTrialSearchRequest = {},
): Promise<SearchBeagleTrialsActionResult> {
  const startedAt = Date.now();
  const { log, requestId } = await createActionLogger({
    action: "searchBeagleTrialsAction",
  });
  log.info(
    {
      event: "start",
    ...
    "search beagle trials action started",
  );

  try {
    const result = await trialsService.searchBeagleTrials(input, { requestId });
    if (!result.body.ok) {
      log.warn(
        {
          event: "failure",
        ...
        "search beagle trials action failed",
      );
      return {
        data: null,
        hasError: true,
    ...
    log.info(
      ...
      "search beagle trials action succeeded",
    );
    ...
    log.error(
      ...
      "search beagle trials action threw",
    );
    return {
      data: null,
      hasError: true,

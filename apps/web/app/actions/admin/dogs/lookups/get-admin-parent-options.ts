...
import { listAdminDogParentOptions } from "@beagle/server";
import { requireAdminLayoutAccess } from "@/lib/server/admin-guard";
import { getSessionCurrentUser } from "@/lib/server/current-user";

export type AdminParentOptionsActionResult = {
  data: AdminDogParentLookupResponse | null;
  hasError: boolean;
  errorCode?: string;
};

export async function getAdminParentOptionsAction(
  input: AdminDogLookupRequest,
): Promise<AdminParentOptionsActionResult> {
  ...
  const result = await listAdminDogParentOptions(input, {
    id: currentUser.id,

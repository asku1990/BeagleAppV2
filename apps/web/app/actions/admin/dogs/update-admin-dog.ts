...
import { updateAdminDog } from "@beagle/server";
import { requireAdminLayoutAccess } from "@/lib/server/admin-guard";
import { getSessionCurrentUser } from "@/lib/server/current-user";

export type UpdateAdminDogActionResult = {
  data: UpdateAdminDogResponse | null;
  hasError: boolean;
  errorCode?: string;
  message?: string;
};

export async function updateAdminDogAction(
  input: UpdateAdminDogRequest,
): Promise<UpdateAdminDogActionResult> {
  ...
  const result = await updateAdminDog(input, {

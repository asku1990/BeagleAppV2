...
import { BEAGLE_TRIALS_PAGE_SIZE_OPTIONS } from "@/lib/public/beagle/trials";
...
export function BeagleTrialsPagination({
  page,
  pageSize,
  total,
  totalPages,
  ...
  return (
    <div className="mt-4 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
      <div className="flex items-center gap-2">
        ...
        >
          {BEAGLE_TRIALS_PAGE_SIZE_OPTIONS.map((size) => (
      ...
      <div className="flex flex-wrap items-center justify-start gap-1 md:gap-2 md:justify-center">
        ...
        {items.map((item, index) =>
          item === "ellipsis" ? (
            ...
            <Button
              ...
              aria-label={`${t("trials.pagination.page")} ${item}`}
            >
              {item}
            </Button>
          ),
        ...
        >
          {t("trials.pagination.next")}
        </Button>
      </div>

      <p className={cn("text-sm md:text-right", beagleTheme.mutedText)}>
        {t("trials.pagination.range")} {start}-{end} / {total}
      </p>
    </div>
  );
}

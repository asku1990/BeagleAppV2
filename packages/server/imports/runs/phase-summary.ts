...
export function formatLegacyImportSummary(
  ...
  switch (input.kind) {
    ...
    case "LEGACY_PHASE1_25":
      return `${phaseLabel}: ${formatMetric("siitosaste", input.siitosasteUpdated)}, ${formatMetric("sairaudet", input.sairaudetInserted)}, ${formatMetric("koiranSairaudet", input.koiranSairaudetInserted)}${input.koiranSairaudetFallbackImported ? `, ${formatMetric("fallbackImported", input.koiranSairaudetFallbackImported)}` : ""}, ${formatMetric("epiLuvut", input.epiLuvutInserted)}, ${formatMetric("errors", input.errorsCount)}.`;
    case "LEGACY_PHASE1_5":

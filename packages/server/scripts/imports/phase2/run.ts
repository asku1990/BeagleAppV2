...
async function main() {
  ...
  console.log("[import:phase2] Starting import...");
  if (createdByUserId) {
    console.log(`[import:phase2] createdByUserId=${createdByUserId}`);
  }

  const result = await importsService.runLegacyPhase2(createdByUserId, {
    log: (message) => console.log(`[import:phase2] ${message}`),
    auditSource: "SCRIPT",
  });
  ...
  console.log(
    `[import:phase2] Completed in ${Math.round((Date.now() - start) / 1000)}s`,
  );
  ...
  if (runId) {
    ...
    if (total > 0) {
      console.log(`[import:phase2] Issue rows stored: ${total}`);
      console.log("[import:phase2] Issue groups:");
      for (const [key, count] of [...grouped.entries()].sort(
        (a, b) => b[1] - a[1],
      )) {
        console.log(`[import:phase2]   ${key} = ${count}`);
      }
      console.log("[import:phase2] Issue samples:");
      for (const sample of samples) {
        console.log(
          `[import:phase2]   [${sample.stage}/${sample.severity}/${sample.code}] reg=${sample.registrationNo ?? "-"} table=${sample.sourceTable ?? "-"} msg=${sample.message}`,
        );
      }
    }
  }
